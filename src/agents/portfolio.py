...
class Portfolio(BaseModel):
    ...
    # Pydantic v2 config style. Context7 /pydantic/pydantic (config docs).
    model_config = ConfigDict()

    cash: float = Field(default=1.5, description="Available cash balance (BTC)")
    initial_capital: float = Field(
        default=1.5,
        description="The starting value of the portfolio for ROI tracking.",
    )
    positions: dict[str, int] = Field(
        default_factory=dict,
        description="Holdings: {item: quantity}",
    )
    realized_pnl: float = Field(
        default=0.0,
        description="Locked-in profit/loss from closed positions",
    )
    ...
    def get_portfolio_value(self, current_prices: dict[str, float]) -> float:
        ...
        position_value = sum(
            qty * current_prices.get(item, 0) for item, qty in self.positions.items()
        )
