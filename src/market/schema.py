...
class Transaction(SQLModel, table=True):
    ...
    timestamp: datetime = Field(
        ...
        description="Time of transaction (UTC)",
    )
...
# Context7 /websites/sqlmodel_tiangolo (SQLModel create tables).
class InteractionLog(SQLModel, table=True):
    ...
    price: float | None = Field(
        default=None,
        description="Price associated with the interaction",
    )
    counterparty_id: str | None = Field(
        default=None,
        description="Counterparty agent ID, if applicable",
    )
